...
final priceList = ['0-99','99-599','600-799','800-999','1000-1499','1500-1999'].obs;
...
final offerSortByList = ['Expert top rated','New Arrivals'].obs;
...
selectedCategoryIdBySingleScreen.value = Get.arguments[1]!??"";
  ...
  if(body!=null){
    if(route.value=='offer'||selectedCategoryIdBySingleScreen.isEmpty){
      ...
      selectedRating.value = body['rating'].isNotEmpty?double.parse(body['rating']):0.0;
      currentRangeValues.value =body['distance'].isNotEmpty? double.parse(body['distance']):0.0;
      ...
      for(int i=0;i<mergeCategoryModel.value.data!.length;i++){
        for(int j=0;j<catList.length;j++){
          if(mergeCategoryModel.value.data![i].id.toString()==catList[j].toString()){
        ...
        for(int k=0;k<mergeCategoryModel.value.data![i].baseCategoryArray!.length;k++){
          for(int m=0;m<subCatList.length;m++){
            if(mergeCategoryModel.value.data![i].baseCategoryArray![k].id.toString()==subCatList[m].toString()){
              mergeCategoryModel.value.data![i].baseCategoryArray![k].isSelectedSubCat.value=true;
              mergeCategoryModel.value.data![i].baseCategoryArray![k].selectedSubCategory.add(subCatList[m].toString());
            }
          }
        }
      }
      ...
      selectedPriceValue.value= body['price'];
      selectedDiscountValue.value = body['discount'];
      selectedSort.value = body['arrivals'].isNotEmpty?2:body['topRated'].isNotEmpty?1:body['hasOffer'].isNotEmpty?0:10;
      selectedRating.value = body['rating'].isNotEmpty?double.parse(body['rating']):0.0;
      currentRangeValues.value =body['distance'].isNotEmpty? double.parse(body['distance']):0.0;
      if(body['subcat'].isNotEmpty){
        subCatList=body['subcat'].split(',').map(int.parse).toList();
      ...
      for(int i=0;i<subCatModel.value.data!.length;i++){
        for(int j=0;j<subCatList.length;j++){
          if(subCatModel.value.data![i].id.toString()==subCatList[j].toString()){
